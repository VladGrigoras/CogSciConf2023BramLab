...
for (i in 1:nrow(selfreport))
{
  ...
  if (selfreport$test[i]==1){
    ...
    prior<-c(selfreport$p1[i], selfreport$p2[i], selfreport$p3[i], selfreport$p4[i], selfreport$p5[i], selfreport$p6[i], selfreport$p7[i], selfreport$p8[i], selfreport$p9[i], selfreport$p10[i], selfreport$p11[i], selfreport$p12[i], selfreport$p13[i], selfreport$p14[i], selfreport$p15[i], selfreport$p16[i], selfreport$p17[i], selfreport$p18[i])
  ...
  selfreport$entropy[i] = -sum(post * log2(post), na.rm=T) #SE(H)
  
  selfreport$information[i] = prior_entropy - selfreport$entropy[i]
  
  ent_alt<- -sum(post_alt * log2(post_alt), na.rm=T)
  
  selfreport$eig[i]<-marg*selfreport$information[i] + (1-marg)*(prior_entropy - ent_alt)
  
  #I left out the eig_max and guessing bit, as this is still incomplete :/ 
}
```